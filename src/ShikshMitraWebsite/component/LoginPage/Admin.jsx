...
const Admin = () => {
    ...
    <div className="  overflow-hidden  w-full mx-auto p-4 md:p-0 flex">
        ...
        <div className="w-full md:w-1/3 p-8 mx-auto sm:bg-gray-100 bg-gray-100 md:bg-gray-50 flex flex-col items-center justify-center">  {/* Added flex and centering classes */}
            ...
            {/* <div className="flex items-center justify-center min-h-screen bg-gray-100 ">
                <div className="  overflow-hidden max-w-4xl w-full mx-auto p-4 md:p-0 flex">
...
"Innovate, Automate, Educate with ease" "A great Saarthi resolves all problems."
                        ...
                        <div className="mt-4">
                            <h3 className="text-lg font-semibold text-gray-700 text-center md:text-left">
                                Vidyalaya  <span className="text-red-500">ERP</span> LOGIN
                            </h3>
...
// import 'react-toastify/dist/ReactToastify.css'; // Import styles if needed

// const Admin = () => {
//   const [formdata, setFormdata] = useState({
//     Username: '',
//     Password: '',
//     Role: 'admin', // or use a dropdown if roles are dynamic
//   });
//   const [loading, setLoading] = useState(false);
//   const [isClosed, setIsClosed] = useState(false); //  Potentially for disabling input while loading
//   const [isLoggedIn, setisLoggedIn] = useState(false); //  Potentially for conditional rendering after login

//   const Navigate = useNavigate();

//   // Toast notifications (replace with your preferred method)
//   const showSuccessToast = (message) => {
//     toast.success(message, {
//       position: toast.POSITION.TOP_CENTER,
...
//   const submitHandler = async (e) => {
//     e.preventDefault();
//     setIsClosed(true); // Disable input while loading
//     setLoading(true);
...
//     } finally {
//       setLoading(false);
//       setIsClosed(false); //Re-enable after the process
//     }
//   };
...
//                 <button
//                   className={`mt-4 bg-indigo-500 text-white py-2 px-6 rounded-lg hover:bg-indigo-600 w-full ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
//                   onClick={submitHandler}
//                   disabled={loading}
//                 >
//                   {loading ? 'Logging In...' : 'Login'}
